<div class="datatable datatable-bordered datatable-head-custom" id="{{ $resource['name'] }}"></div>

--at--section('scripts')
    <script>
        (() => {
            const {{ $resource['name']  }} = function () {
                const _default = () => {
                    const datatable = $('#{{ $resource['name'] }}').KTDatatable({
                        data: {
                            type: 'remote',
                            source: {
                                read: {
                                    url: APP_URL + '{{ $resource['api'] }}',
                                },
                            },
                            pageSize: 10,
                            serverPaging: true,
